...
<body>
	...
	<br>
	<font size="5" id="point">포인트</font> /
	<font size="5" id="heart">목숨</font>
	...
	<script type="text/javascript">
		...
		document.getElementById('heart').innerHTML = "목숨 : "
				+ heart_counter(heart);
		function gamewin() {
			...
			if (level == 2) { // 최종 레벨을 클리어하면 승리 (임의로 수정 가능)
				...
				var message = "<h1>" + (parseInt(nextlevel) - 1)
						+ "단계 성공!</h1><br><a href='?level=" + nextlevel
						+ "'>다음 스테이지</a>";
		...
		function gameover(code) {
			...
			document.body.innerHTML = message
					+ "<button onclick='location.reload()'><h3>다시하기</h3></button>";
		}
		...
		function keydown(keyCode) {
			if (keyCode == 13) {
				...
				if (keyword.indexOf(text.value) != -1) { // 입력한 값이 키워드에 있으면 실행
					...
					keyword_cnt -= 1;
					point += 100;
					document.getElementById('point').innerHTML = "포인트 : "
							+ point;
				}
				text.value = "";
				if (point >= goal) {
