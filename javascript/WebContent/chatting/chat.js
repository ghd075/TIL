...
chat.ChatUI.prototype = { //채팅창 디자인 변경할려면 UI변경
	...
	render: function() { //채팅창을 그려주는 부분
		this.makeLoginWindow();
		this.makeChatWindow();
	},
	//로그인창 UI
	makeLoginWindow: function() {
		var loginWinDiv = document.createElement("div"); //div태그 생성
		var nickNameInput = document.createElement("input"); //input태그 생성
		nickNameInput.type = "text"; //input태그 type = 'text'
		nickNameInput.size = "10"; //input태그 size = '10'
	
		var loginBtn = document.createElement("input"); //input태그 생성
		loginBtn.type = "button"; //input태그 type = 'button'
		loginBtn.value = "로그인"; //input태그 value = '로그인'
