...
public class MemberListController implements Controller {
	...
	public String execute(HttpServletRequest request, HttpServletResponse response)
		...
		paging.setTotalRecord(HjhDAO.getInstance().count_m(desc)); //HjhDAO에서 전체 레코드 건수를 기록한다
		
		int first = paging.getFirst();
		int last = paging.getLast();
		
		list = HjhDAO.getInstance().MemberList(first, last, desc); // HjhDAO에서 넘어오는 변수들을 리스트에 담는다.
